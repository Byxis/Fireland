...
public abstract class AbstractStorage
{
    ...
    private final Map<Integer, Inventory> m_activePages;
    ...
    private void setNavigationItems(Inventory inv, int currentPage)
    {
        ...
        if (currentPage > 0)
        {
            inv.setItem(PREV_PAGE_SLOT, InventoryUtilities.setItemMetaLore(Material.ARROW, "§a← Page précédente", (short) 0,
        ...
        if (totalPages > 1)
        {
            inv.setItem(PAGE_INFO_SLOT,
                    InventoryUtilities.setItemMetaLore(Material.BOOK, "§6Page " + (currentPage + 1) + "/" + totalPages, (short) 0,
                            List.of("§7Vous consultez la page " + (currentPage + 1), "§7Total d'emplacements: §e" + getMaxSpaceStorage(),
                                    "§7Emplacements sur cette page: §e" + getPageSize(currentPage))));
        }

        if (currentPage < totalPages - 1)
        {
            inv.setItem(NEXT_PAGE_SLOT, InventoryUtilities.setItemMetaLore(Material.ARROW, "§aPage suivante →", (short) 0,
    ...
    public int getCurrentPage(String inventoryTitle)
    {
        if (inventoryTitle.contains("(") && inventoryTitle.contains("/"))
        {
            try
            {
                String pageInfo = inventoryTitle.substring(inventoryTitle.lastIndexOf("(") + 1, inventoryTitle.lastIndexOf(")"));
    ...
    private boolean isNavigationItem(ItemStack item)
    {
        ...
        return type == Material.GRAY_STAINED_GLASS_PANE || type == Material.WHITE_STAINED_GLASS_PANE
                || type == Material.RED_STAINED_GLASS_PANE || type == Material.ARROW || type == Material.BARRIER || type == Material.BOOK;
    }
    ...
    private ItemStack loadItemFromDatabase(int index)
    {
        ...
        try (Connection connection = firelandConnection.getConnection())
        {
            final PreparedStatement preparedStatement = connection
                    .prepareStatement("SELECT item FROM " + m_tableName + " WHERE owner = ? AND index_item = ?");
    ...
    private void saveItemToDatabase(int index, ItemStack item)
    {
        ...
        try (Connection connection = firelandConnection.getConnection())
        {
            final PreparedStatement checkStatement = connection
                    .prepareStatement("SELECT 1 FROM " + m_tableName + " WHERE owner = ? AND index_item = ?");
            ...
            if (exists)
            {
                final PreparedStatement updateStatement = connection
                        .prepareStatement("UPDATE " + m_tableName + " SET item = ? WHERE owner = ? AND index_item = ?");
            ...
            else
            {
                final PreparedStatement insertStatement = connection
                        .prepareStatement("INSERT INTO " + m_tableName + "(owner, index_item, item) VALUES(?, ?, ?)");
    ...
    private void removeItemFromDatabase(int index)
    {
        ...
        {
            final PreparedStatement preparedStatement = connection
                    .prepareStatement("DELETE FROM " + m_tableName + " WHERE owner = ? AND index_item = ?");
            preparedStatement.setString(1, m_name);
            preparedStatement.setInt(2, index);
