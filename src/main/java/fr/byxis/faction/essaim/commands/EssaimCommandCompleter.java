...
/**
 * Completer for the "essaim" command, providing tab completion suggestions based on the command context.
 */
public class EssaimCommandCompleter implements TabCompleter
{
    ...
    @Override
    public List<String> onTabComplete(@NotNull CommandSender _sender, @NotNull Command _command,
                                      @NotNull String _alias, String[] _args)
    {

        if (_args.length == 1)
        {
            return ListUtilities.filterSuggestions(_args[0], Arrays.asList("spawner", "create", "open", "close",
                    "remove", "set", "join", "finish", "unfinish", "info", "reset", "setblock", "forcejoin", "forcekick"));
        }
        ...
        if (_args.length == 2)
        {
            switch (_args[0].toLowerCase())
            {
                ...
                case "unfinish":
                case "remove":
                    return ListUtilities.filterSuggestions(_args[1], new ArrayList<>(m_fireland.getEssaimManager().getConfigService().getEssaimNames()));
                case "set":
                    return ListUtilities.filterSuggestions(_args[1], Arrays.asList("hub", "start", "reset", "entry", "solo", "key",
    ...
    /**
     * Retrieves a list of active essaim names from the EssaimManager.
     * It first checks the GroupManager for active groups, and if none are found,
     * it falls back to checking the EssaimConfigService and EssaimService for active essaims.
     *
