 ...
 * </ul>
 * <p>
 * The enum also provides utility methods to parse cooldown types and durations from strings.
 * </p>
 * <p>
 * Example usage:
 * <pre>
...
public enum CooldownType
{
    ...
    DYNAMIC_DAY("Nd", null),
    DYNAMIC_WEEK("Nw", null),
    DYNAMIC_MONTH("Nm", null),
    NONE("none", null);

    private final static CooldownType DEFAULT = DYNAMIC_WEEK;
    private final static Pattern DURATION_PATTERN = Pattern.compile("(\\d+)([dwm])");
    ...
    /**
     * Parses a string to determine the corresponding CooldownType.
     * Supported formats:
     * - "first-time" for FIRST_TIME
     * - "none" for NONE
     * - "Nd", "Nw", "Nm" for DYNAMIC_DAY, DYNAMIC_WEEK, DYNAMIC_MONTH respectively, where N is a positive integer.
     * If the input does not match any known format, DEFAULT (DYNAMIC_WEEK) is returned.
     *
     * @param value the string representation of the cooldown type
     * @return the corresponding CooldownType
     */
    public static CooldownType fromString(String value)
    {
        if (value == null) return DEFAULT;
        if ("first-time".equals(value)) return FIRST_TIME;
        ...
        if (matcher.matches())
        {
            ...
            switch (unit)
            {
                case "d": return DYNAMIC_DAY;
                case "w": return DYNAMIC_WEEK;
                case "m": return DYNAMIC_MONTH;
            }
        }
     ...
     * - "Nd" for N days
     * - "Nw" for N weeks
     * - "Nm" for N months (approximated as 30 days)
     * If the input is null or does not match any known format, a default duration of 7 days is returned.
     *
    ...
    public static Duration getDuration(String value)
    {
        ...
        if (matcher.matches())
        {
            ...
            switch (unit)
            {
                ...
                case "w": return Duration.ofDays(amount * 7L);
                case "m": return Duration.ofDays(amount * 30L);
            }
        }
    ...
    /**
     * Extracts the numeric amount from a duration string.
     * Supported formats:
     * - "Nd" for N days
     * - "Nw" for N weeks
     * - "Nm" for N months
     * If the input is null or does not match any known format, a default amount of 1 is returned.
     *
     * @param value the string representation of the duration
