 ...
 * <ul>
 * <li>Event-based cooldowns (for event essaims with delay between
 * participations)</li>
 * <li>Participation cooldowns (short-term cooldown after completing an
 * essaim)</li>
...
public class EssaimCooldownService
{
     ...
     *
     * @param _repository
     *            The repository for managing cooldown data
     */
     ...
     * <p>
     * For event-based essaims, checks if the player has completed the event
     * cooldown period. For regular essaims, checks if the participation cooldown
     * has expired.
     *
     * @param _player
     *            The player to check
     * @param _essaim
     *            The essaim to check entry for
     *
    ...
    public boolean canPlayerEnter(Player _player, EssaimClass _essaim)
    {
        ...
        {
            return !m_repository.isPlayerInCooldown(_player.getUniqueId(), _essaim.getName(), _essaim.getDelayBetweenEvents());
        }
        return canPlayerParticipate(_player, _essaim.getName());
    }

    /**
     * Checks if a player can participate in an essaim based on participation
     * cooldown.
     * <p>
     * Players must wait {@value #PARTICIPATION_COOLDOWN_HOURS} hour(s) after
     * completing an essaim before they can participate again.
     *
     * @param _player
     *            The player to check
     * @param _essaimName
    ...
    {
        LocalDateTime lastParticipation = m_repository.getLastCompletionDate(_player.getUniqueId(), _essaimName);
     ...
     * <p>
     * Calculates how many days remain before the player can participate in the
     * event essaim again.
     *
     * @param _player
     *            The player to check
    ...
    public long getRemainingCooldownDays(Player _player, EssaimClass _essaim)
    {
        ...
        LocalDateTime lastCompletion = m_repository.getLastCompletionDate(_player.getUniqueId(), _essaim.getName());
     ...
     * <p>
     * Calculates how many minutes remain before the player can participate in the
     * essaim again after their last completion.
     *
     * @param _player
     *            The player to check
     * @param _essaimName
     ...
     *            The essaim name
     * @param _rewardType
     *            The type of reward (e.g., "jetons", "command")
     * @param _rewardId
     *            The unique identifier for this reward
     * @param _cooldownValue
     *            The cooldown configuration value (e.g., "1w", "first-time",
     *            "none")
     *
     ...
     * <p>
     * Compares the last reward date with the current time, taking into account the
     * specified cooldown duration.
     *
     * @param _playerUuid
     *            The player's UUID
     * @param _essaimName
     ...
     * @param _cooldownValue
     *            The cooldown configuration value
     *
