...
public class DataZone
{
    ...
    private void initZones()
    {
        ...
        for (String zone : configManager.getConfig().getConfigurationSection("zone").getKeys(false))
        {
            Location loc = new Location(Bukkit.getWorld(configManager.getConfig().getString("zone." + zone + ".world")),
                    configManager.getConfig().getDouble("zone." + zone + ".x"), configManager.getConfig().getDouble("zone." + zone + ".y"),
                    configManager.getConfig().getDouble("zone." + zone + ".z"));
            ZoneClass zoneClass = new ZoneClass(zone, loc, configManager.getConfig().getInt("zone." + zone + ".teleportation-price"),
    ...
    private void saveClaiming(String factionName, Timestamp claimedAt, String zone)
    {
        ...
        try (Connection connection = firelandConnection.getConnection())
        {
            ...
            PreparedStatement isInDb = connection
                    .prepareStatement("SELECT capture_zone.faction_name FROM capture_zone WHERE capture_zone.zone = ?");
            isInDb.setString(1, zone);
            ResultSet rs = isInDb.executeQuery();
            if (rs.next())
            {
                final PreparedStatement ps = connection
                        .prepareStatement("UPDATE capture_zone SET faction_name = ?, capture_time = ?, rewarded_at = ? WHERE zone = ?");
            ...
            {
                final PreparedStatement ps = connection
                        .prepareStatement("INSERT INTO capture_zone(zone,faction_name,capture_time,rewarded_at) VALUES (?,?,?,?)");
                ps.setString(1, zone);
    ...
    public void saveTiming(String factionName, Timestamp claimedAt, String zone)
    {
        ...
        try (Connection connection = firelandConnection.getConnection())
        {
            PreparedStatement isInDb = connection.prepareStatement(
                    "SELECT faction_zone.duration FROM faction_zone WHERE faction_zone.zone = ? AND faction_zone.faction_name =?");
            isInDb.setString(1, zone);
            ...
            {
                final PreparedStatement ps = connection
                        .prepareStatement("UPDATE faction_zone SET faction_zone.duration = ? WHERE zone = ? AND faction_name =?");
            ...
            {
                final PreparedStatement ps = connection
                        .prepareStatement("INSERT INTO faction_zone(faction_name,zone,duration) VALUES (?,?,?)");
                ps.setString(2, zone);
                ps.setString(1, factionName);
                ps.setLong(3, diff);
                ps.executeUpdate();
    ...
    void removeSavedClaiming(String zone, String faction)
    {
        ...
        {
            // Préparation de la commande
            final PreparedStatement isInDb = connection
                    .prepareStatement("SELECT faction_name, capture_time FROM capture_zone WHERE zone = ?");
            isInDb.setString(1, zone);
            ...
            {
                /*
                 * if (faction != null && !faction.equalsIgnoreCase("")) { final
                 * PreparedStatement hasEverClaimedThisZone = connection.
                 * prepareStatement("SELECT duration FROM faction_zone WHERE zone = ? AND faction_name = ?"
                 * ); hasEverClaimedThisZone.setString(1, zone);
                 * hasEverClaimedThisZone.setString(2, faction); ResultSet result =
                 * hasEverClaimedThisZone.executeQuery(); if (rs.next()) { final
                 * PreparedStatement update = connection.
                 * prepareStatement("UPDATE faction_zone SET duration = ? WHERE faction_name = ? AND zone = ?"
                 * ); update.setInt(1, result.getInt(1) + (int) ((System.currentTimeMillis() -
                 * rs.getTimestamp(2).getTime()) + rs.getTimestamp(2).getTime()));
                 * update.setString(2, zone); update.setString(3, faction);
                 * update.executeUpdate(); } else { final PreparedStatement update =
                 * connection.prepareStatement("INSERT INTO faction_zone VALUES (?,?,?)");
                 * update.setString(1, zone); update.setString(2, faction); update.setInt(3,
                 * (int) ((System.currentTimeMillis() - rs.getTimestamp(2).getTime()) +
                 * rs.getTimestamp(2).getTime())); update.executeUpdate(); } }
                 */
    ...
    private void actualizeClaiming()
    {
        ...
        try (Connection connection = firelandConnection.getConnection())
        {
            ...
            {
                final PreparedStatement isInDb = connection
                        .prepareStatement("SELECT zone,faction_name,capture_time FROM capture_zone WHERE zone = ?");
    ...
    private void setRewardNumber(ZoneClass zone, Timestamp time)
    {
        ...
        try (Connection connection = firelandConnection.getConnection())
        {
            ...
            final PreparedStatement insert = connection
                    .prepareStatement("UPDATE capture_zone SET rewarded_at = ? WHERE zone = ? AND faction_name = ? ");
    ...
    {
        if (m_zoneEnter.get(zone).isEmpty())
            return;
